module SimulateGPUGeneralCoords
...
"""
function simulate_ensemble_bulk_gpu_general_coords(
    ...
    try
        ...
        @cuda threads=threads blocks=blocks kernel_boost_to_lab_frame_general_coords_gpu!(
        ...
        #p_mags = abs.(momenta[2, :])
        momenta_history_gpu[:, 1] .= momenta[2,:]

        position_history_gpu = CUDA.zeros(Float64,1, N_particles, num_saves + 1)
        position_history_gpu[1,:, 1] .= positions[2,:]
        ...
        @showprogress 10 "Running Langevin simulation GPU Cartesian..." for step in 1:steps
            ...
            @cuda threads=threads blocks=blocks kernel_boost_to_rest_frame_general_coords_gpu!(
            ...
            @cuda threads=threads blocks=blocks kernel_compute_all_forces_general_coords_gpu!(
            ...
            @cuda threads=threads blocks=blocks kernel_update_positions_general_coords_gpu!(
            ...
            if step % save_every == 0
                ...
                @cuda threads=threads blocks=blocks kernel_save_positions_general_coords_gpu!(
